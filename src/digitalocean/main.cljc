(ns digitalocean.main
  "Kotodama WASM entrypoint for the DigitalOcean clean-room actor (L4) — Clojure port.
  ...
  facts (`digitalocean.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "digitalocean")
(def tier "L4")
...
(def entity-specs
  [{:entity "Droplet"   :plural "droplets"   :id-prefix "digitalo_dro"
    :fields [:name :status :memory :vcpus :disk :locked]              :required [:name]
    :coerce {:memory :int :vcpus :int :disk :int :locked :bool}       :refs {}}
   {:entity "Volume"      :plural "volumes"      :id-prefix "digitalo_vol"
    :fields [:name :sizeGigabytes :filesystemType]        :required [:name]
    :coerce {:sizeGigabytes :int}          :refs {}}
   {:entity "DatabaseCluster"     :plural "databaseclusters"     :id-prefix "digitalo_dat"
    :fields [:name :engine :numNodes :size :region :status]        :required [:name]
    :coerce {:numNodes :int}       :refs {}}
   {:entity "LoadBalancer" :plural "loadbalancers" :id-prefix "digitalo_loa"
    :fields [:name :ip :status :sizeUnit]          :required [:name]
    :coerce {:sizeUnit :int}        :refs {}}
   {:entity "KubernetesCluster"   :plural "kubernetesclusters"   :id-prefix "digitalo_kub"
    :fields [:name :region :version :endpoint :autoUpgrade :ha]   :required [:name]
    :coerce {:autoUpgrade :bool :ha :bool}       :refs {}}
   {:entity "Image"    :plural "images"    :id-prefix "digitalo_ima"
    :fields [:name :slug :public :status :type :sizeGigabytes]  :required [:name]
    :coerce {:public :bool :sizeGigabytes :float}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"digitalocean.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "digitalocean-compat" :tier tier :entities entities} 200])
...
(defn start! [] :digitalocean-compat/ready)
